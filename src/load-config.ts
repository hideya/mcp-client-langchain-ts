...
export function loadConfig(path: string): Config {
  try {
    ...
    // Normalize alias: convert "model_provider" to "provider"
    if (config.llm && "model_provider" in config.llm) {
      if ("provider" in config.llm) {
        throw new Error("Cannot specify both `provider` and `model_provider`");
      }
